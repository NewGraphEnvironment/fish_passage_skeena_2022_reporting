...
`r fpr_my_habitat_paragraph(loc = 'ds')` This stretch of stream was steep but had good habitat. There was one lock block step approximately 30m downstream of the culvert that was approximately 1m in height that could block upstream migration for non adult fish species `r if(gitbook_on){knitr::asis_output("(Figure \\@ref(fig:photo-198215-02)).")}else(knitr::asis_output("(Figure \\@ref(fig:photo-198215-d01))."))` This was likely put in this location in the past to act as a backwatering structure and attempt to minimize the culvert outlet drop to facilitate upstream fish passage. The
...
`r fpr_my_habitat_paragraph(loc = 'us')` There were numerous cascades that were approximately 1m in height at the time of survey, with deep outlet pools. The stream channel was frequently confined and restriced from lateral migration from the valley walls. Significant amounts of large woody debris were found in and around the stream, adding complexity. There were trace amount of gravels suitable for spawning. After the first 500m of the survey, there were logs deliberately placed perpendicular to the stream at regular intervals (shown in `r if(gitbook_on){knitr::asis_output("Figure \\@ref(fig:photo-198215-06).)")}else(knitr::asis_output("Figure \\@ref(fig:photo-198215-d04)).")` These most likely function to capture substrate and provide complexity and spawning/rearing habitat.  The stream was surveyed to a location of a dam listed in the Canadian Aquatic Barrier Database [@canadianwildlifefederation2023CanadianAquatic]. The dam was not present but there appeared to be some remnants left (Figure \@ref(fig:photo-198215-05)).  The habitat was rated as
...
The BC freshwater atlas streamline indicates the stream gradient was greater than 9% below the location of the dams. After surveying the stream, measurements indicate the average gradient was below the 8% cutoff which determines accessible steelhead habitat. Therefore, there is 0.8km of habitat suitable for steelhead rearing. Areas surveyed were rated as `r fpr::fpr_my_priority_info(sit = my_site, loc = 'us', col_pull = hab_value)`
value for salmonid rearing and spawning. `r as.character(my_site)` was ranked as a
`r fpr::fpr_my_priority_info(col_pull = priority)` priority for proceeding to design for replacement. Electrofishing upstream of the culvert is recommended to provide insight into fish community
...
```{r photo-198215-d04, fig.cap = my_caption, fig.show="hold", out.width= c("49.5%","1%","49.5%"), eval=identical(gitbook_on, FALSE)}
