...
`r fpr::fpr_my_pscis_info()`. The site is located on `r fpr_my_pscis_info(col_pull = road_name)` (also known as Dale Creek FSR), just outside the Gitxsan village of Kispiox. Crossing `r as.character(my_site)` was located
...
for the area upstream of crossing `r as.character(my_site)`[@moe2020KnownBC; @norris2020bcfishobs].
...
`r fpr_my_habitat_paragraph(loc = 'us')` There were numerous cascades that were approximately 1m in height at the time of survey, with deep outlet pools. The stream was wide throughout. A lot of large woody debris was found in and around the stream. There were trace areas with gravels that could be suitable for spawning. After the first 500m of the survey, there were logs deliberately placed perpendicular to the stream at regular intervals (shown in Figure \@ref(fig:photo-198215-06)). These most likely function to capture substrate, provide complexity and spawning/rearing habitat. The habitat was rated as
