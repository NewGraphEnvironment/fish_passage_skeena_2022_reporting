...
\@ref(fig:plot-fish-box-198217). A total of `r tab_fish_summary_2023 %>% filter(site_id == paste0(my_site2, '_us')) %>% group_by(site_id) %>% janitor::adorn_totals() %>% filter(site_id == 'Total') %>% pull(count_fish)` fish were captured upstream and `r tab_fish_summary_2023%>% filter(site_id == paste0(my_site2, '_ds')) %>% group_by(site_id) %>% janitor::adorn_totals() %>% filter(site_id == 'Total') %>% pull(count_fish)` fish were captured downstream `r if(gitbook_on){knitr::asis_output("(Figures \\@ref(fig:photo-198217-07) - \\@ref(fig:photo-198217-08)).")}else(knitr::asis_output("(Figures \\@ref(fig:photo-198217-u01))."))` Dolly varden and cutthroat trout were captured downstream of the crossing, but only cutthroat trout were captured upstream. All fish captured with a fork length greater than 60mm were tagged with Passive Integrated Transponders (PIT tags) with data stored [here](`r paste0(params$repo_url, "blob/main/data/2023/habitat_confirmations.xls")`).
