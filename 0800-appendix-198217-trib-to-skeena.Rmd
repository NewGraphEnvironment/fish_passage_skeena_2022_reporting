...
\@ref(tab:tab-culvert-bcfp-198216). Note that the BC freshwater atlas does not capture the upstream watershed, therefore the modelling is likely inaccurate. A map of the watershed is provided
in map attachment
...
```{r tab-culvert-bcfp-198217, eval = F}
...
crossing `r as.character(my_site2)` with a bridge
(`r fpr::fpr_my_pscis_info(site = my_site2, col_pull = recommended_diameter_or_span_meters)`m
...
value for coho rearing and spawning. Beaver management and riparian restoration are important steps to the conservation of this system. A plan to conserve and rehabilitate this system is being layed out by the Gitxsan Watershed Authority, in collaboration with the local community.   

<br>

Recommendations going forward include:

* Electrofishing and PIT tagging upstream of crossing `r as.character(my_site2)` and within the northern tributary is recommended to provide insight into fish
community composition and density.

* Acquisition of lidar data for areas to the west of current data available on LidarBC (includes area of water intake facilities) to help inform about the remnant channel locations

* Acquisition of temperature data at points likely to contain water all year long (ex. outlet of culverts and upstream of culvert on water tower road). Acquire data for the entire growing season and include areas from main north channel to elevations where coho are expected to populate. We define the growing season as per @coleman_fausch2007ColdSummer, which delineate it as the beginning of the first week that average stream temperatures exceed and remain above 5$^\circ$C in the spring and the end of the growing season as the last day of the first week that average stream temperature drops below 4$^\circ$C. We specified Monday as the first day of the week and identified the week of the year based on the number of 7 day periods from January 1 plus 1.  Celsius degree-days were calculated as the mean daily temperature (ex. 2 days with average temperatures of 7$^\circ$C with sum to 14 Celsius degree-days).

* Delineation of reaches and acquisition of FHAP data developed by @johnston_slaney1996FishHabitat within all reaches expected to be populated by coho.
...
* Overall outreach program to educate Kispiox residents about the role of beavers in watershed health and the history of impacts to aquatic values due to road, community, and ranch development in the valley

* Acquisition of sufficient hydrometric information in accordance with the Manual of British Columbia Hydrometric Standards [@resourcesinformationstandardscommittee2018ManualBritish] to build stage discharge curves for the watershed so the relationships between fish life history timings, physical habitat suitability, temperature and flows can be understood
