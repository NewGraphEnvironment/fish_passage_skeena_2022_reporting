...
Electrofishing was conducted at crossing `r as.character(my_site2)` in 2023 with results summarised in Tables \@ref(tab:tab-fish-site-198217) - \@ref(tab:tab-fish-dens-198217) and Figure
\@ref(fig:plot-fish-box-198217). A total of `r tab_fish_summary_updated %>% filter(site_id == paste0(my_site2, '_us')) %>% group_by(site_id) %>% janitor::adorn_totals() %>% filter(site_id == 'Total') %>% pull(count_fish)` fish were captured upstream and `r tab_fish_summary_updated %>% filter(site_id == paste0(my_site2, '_ds')) %>% group_by(site_id) %>% janitor::adorn_totals() %>% filter(site_id == 'Total') %>% pull(count_fish)` fish were captured downstream `r if(gitbook_on){knitr::asis_output("(Figures \\@ref(fig:photo-198217-07) - \\@ref(fig:photo-198217-08) - \\@ref(fig:photo-198217-09)).")}else(knitr::asis_output("(Figures \\@ref(fig:photo-198217-u01) - \\@ref(fig:photo-198217-u02))."))` Although dolly varden and cutthrout trout were captured downstream of the crossing, only cutthrout trout were captured upstream. 
...
fpr_table_fish_site(dat = tab_fish_sites_sum_updated, sit = my_site2)

```

<br>

```{r tab-fish-dens-198217, eval=F}
fpr_table_fish_density(dat = fish_abund_updated, sit = my_site2)

```

`r if(gitbook_on){knitr::asis_output("<br>")} else knitr::asis_output("\\pagebreak")`

```{r plot-fish-box-198217, fig.cap= my_caption, eval=F}
my_caption <- paste0('Densites of fish (fish/100m2) captured upstream of PSCIS crossing ', my_site2, '.')

fpr_plot_fish_box(dat = fish_abund_updated, sit = my_site2) 
...
my_photo3 = fpr::fpr_photo_pull_by_str(site = my_site2, dir_photos = "data/photos/", str_to_pull = '_ct_ds')

my_caption3 = paste0('Cutthroat trout captured downstream of PSCIS crossing ', my_site2, '.')

```

```{r photo-198217-09, fig.cap= my_caption3, out.width = photo_width, eval=gitbook_on}
grid::grid.raster(jpeg::readJPEG(my_photo3))
```

<br>

```{r photo-198217-u01, fig.cap = my_caption, fig.show="hold", out.width= c("49.5%","1%","49.5%"), eval=identical(gitbook_on, FALSE)}

my_caption <- my_caption1
knitr::include_graphics(my_photo1)
```

<br>

```{r photo-198217-u02, fig.cap = my_caption, fig.show="hold", out.width= c("49.5%","1%","49.5%"), eval=identical(gitbook_on, FALSE)}

my_caption <- paste0('Left: ', my_caption2, ' Right: ', my_caption3)

knitr::include_graphics(my_photo2)
knitr::include_graphics("fig/pixel.png")
knitr::include_graphics(my_photo3)
```
