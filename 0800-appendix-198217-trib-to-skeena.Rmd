...
are located on `r fpr::fpr_my_pscis_info()`, just outside the small
community of Glen Vowell, BC. Crossing `r as.character(my_site)` is
located on `r fpr_my_pscis_info(col_pull = road_name)` and
`r as.character(my_site2)` is located on
`r fpr_my_pscis_info(site = my_site2, col_pull = road_name)`. Crossing
`r as.character(my_site2)` was located
`r round(fpr::fpr_my_bcfishpass(site = my_site2, col_pull = downstream_route_measure)*0.001, 1)`km
upstream from the confluence with the Skeena River, and crossing
`r as.character(my_site)` was located 300m further upstream. Crossing
...
fisheries information available for the area upstream of crossing
...
The Glen Vowell Indian Band (also known as Sik-e-Dakh), is a small
community of the Gitxsan First Nations. They reside on the west side of
the Skeena River, between Hazelton and Kispiox. Their reserve land spans
637.70ha [@bcassemblyoffirstnations2023]. In a personal communication with a local chief (Francis), it was found that the local community used to harvest coho from the stream in periods of low flow. 
...
my_photo1 = fpr::fpr_photo_pull_by_str(site = my_site2, str_to_pull = '_trashrack_')

my_caption1 = paste0('Trash rack on inlet side of PSCIS crossing ', my_site2, '.')
...
suitable for coho, steelhead, and rainbow spawning. Local knowledge of
historic coho spawning in system. Multiple large beaver dams were
present in site. The stream flows adjacent to the community of Glen
Valle and supplies community drinking water. Upstream end of site was
upstream of a beaver dam complex on southern tributary. The habitat was
rated as `r fpr::fpr_my_priority_info(loc = 'us')` value for salmonid
rearing and spawning.

<br>

## Stream Characteristics Downstream of `r as.character(my_site)` and Upstream of `r as.character(my_site2)` {.unnumbered}
...
value for salmonid rearing and spawning. Electrofishing upstream and
